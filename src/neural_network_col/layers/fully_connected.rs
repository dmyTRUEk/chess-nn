...
impl Layer for FullyConnected {
    fn forward_propagation(&self, input: Vector) -> Vector {
        let output: Vector = &self.weights_matrix.transpose() * input + &self.shift_vector;
        output
    }
    ...
    fn backward_propagation(&mut self, output_error: Vector, learning_rate: float) -> Vector {
        ...
        let input_error: Vector = &self.weights_matrix * &output_error;
        let weights_error: Matrix = &output_error * self.input.as_ref().unwrap().transpose();
        // println!("weights_error: {:?}", weights_error);
        // println!("input_error: {:?}", input_error);

        // update self params
        self.weights_matrix -= learning_rate * weights_error.transpose();
